...
from backend_apu.sql_validator import validate_readonly_query as _validate_readonly_query
from apu_extractor.ai_provider import generate_text as ai_generate

log = logging.getLogger("mapus.whatsapp")
router = APIRouter()
...
SQL:"""
    ...
    is_valid, validated_sql = _validate_readonly_query(sql_query)
    if not is_valid:
...
"""
    ...
    guardar_conversacion(from_number, message_body, sql_query if is_valid else "", respuesta)
