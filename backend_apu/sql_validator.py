...
log = logging.getLogger("mapus.sql")
...
def validate_readonly_query(sql: str) -> Tuple[bool, str]:
    """Validate that a SQL string is a read-only SELECT/WITH query against allowed tables.
    
    Returns (True, validated_sql) on success, or (False, error_message) on failure.
    Enforces: no multi-statement, no dangerous keywords/functions, only allowed tables,
    no SELECT *, and a maximum LIMIT of {MAX_LIMIT}.
    """
    ...
    if has_limit:
        ...
        if limit_value > MAX_LIMIT:
            sql = re.sub(
                r"\blimit\s+\d+", f"LIMIT {MAX_LIMIT}", sql, flags=re.IGNORECASE
            )
    else:
